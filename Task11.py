...
def max_mul(table: List[List[int]], row: int = 4) -> int:
    current_max = -1
    n = len(table)
    ...
    for i in range(0, n):
        for j in range(0, n-row+1):
            ...
            for k in range(j, j+row):
                mul_h *= table[i][k]
                mul_v *= table[k][i]

            if mul_h > current_max:
                current_max = mul_h
            if mul_v > current_max:
                current_max = mul_v

    for k in range(0, n * 2 - 1):  # k - сумма индексов
        diag = []
        r_diag = []
        for i in range(0, k+1):
            ...
            r_diag.append(table[i][r_j])

        for i in range(0, len(diag)-row+1):
            mul_diag = 1
            mul_r_diag = 1
            ...
            if mul_diag > current_max:
                current_max = mul_diag
            if mul_r_diag > current_max:
                current_max = mul_r_diag
